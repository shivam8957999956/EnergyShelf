...
export default function PostSingle({ post }) {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const res = await axiosInstance.get("/users/?user=" + post.username);
  ...
  const handleBook = async () => {
    if (user) {
      const res = await axiosInstance.put("/posts/bookmark/" + post._id, {
        username: post.username,
        bookmark: user._id,
        check: book,
      });
  ...
  const handleLike = async () => {
    if (user) {
      try {
        ...
        // const res1 = await axiosInstance.put("/profile/like/" + post.userId, {
        //   check: like,
        // });
        // console.log(res1);
        // console.log(like);
