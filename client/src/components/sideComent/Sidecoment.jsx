...
export default function Sidecoment() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user) {
      ...
      try {
        const res = await axiosInstance.post("/comment", newPost);
        console.log(res);
        setCheck([]);
        fetchPosts();
        setDesc("");
  ...
  const handleReport = async (com) => {
    if (com.totalReport.length >= 0) {
      ...
      const res = await axiosInstance.put("/comment/" + com._id, {
        reportUser: user._id,
      });
    }
    // console.log(res);
  ...
  const handleDelete = async (com) => {
    ...
    const res = await axiosInstance.delete("/comment/" + com._id);
    fetchPosts();
  };
  return (
    <>
