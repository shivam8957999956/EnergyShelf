...
export default function Recommend({ p, userId }) {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      console.log(p);
      const res1 = await axiosInstance.get("/posts/?user=" + p);
      setRecommend(res1.data);
      // const res = await axiosInstance.put("/profile/views/" + userId, {});
      // console.log(res);
    };
    fetchPosts();
    const getPost = async () => {
  ...
  const fetchPosts = async () => {
    console.log(p);
    const res1 = await axiosInstance.get("/posts/?cat=" + "general");
    setRecommend(res1.data);
    //   console.log(res1);
  };
