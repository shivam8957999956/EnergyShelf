...
export default function Home() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      setPosts(res.data);
      setLoading(false);
      const res1 = await axiosInstance.get("/popularity/");
      setPopularAll(res1.data);
      const res2 = await axiosInstance.get("/popularity/likes");
      setLikeAll(res2.data);
    };
    fetchPosts();
  }, [search]);
  const handleClick = async () => {
    const res = await axiosInstance.get("/popularity/all");
    setPosts(res.data);
  ...
  const handleQuiz = async () => {
    // window.location.replace("/");
    const res = await axiosInstance.get("/posts/?quiz=true");
    setPosts(res.data);
  };
  ...
  const handleUpdate = async (post) => {
    try {
      ...
      const res1 = await axiosInstance.put("/profile/views/" + post.userId, {});
      // console.log(res1);
    } catch (err) {}
  };
  return (
